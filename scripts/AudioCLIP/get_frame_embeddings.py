...
def load_audio_clip_model():
    model_path = "scripts/AudioCLIP/assets/AudioCLIP-Full-Training.pt"
...
def extract_patches_rect(image, patch_size, patches_per_row, patches_per_column):
    ...
    stride_x = 1 if (stride_x == 0) else stride_x
    stride_y = (height - patch_size) // (patches_per_column - 1)
    stride_y = 1 if (stride_y == 0) else stride_y
...
if __name__ == "__main__":
    ...
    FrameArgs.video = "scripts/examples/band.mp4"
    frame,image = main()

    # from get_audio_embeddings import Result
    if not FrameArgs.audio:
        audio1 = torch.load("scripts/embeddings/alan_dirt.pt")
    ...
    axs[0].imshow(frame[0]@audio, cmap='jet')
